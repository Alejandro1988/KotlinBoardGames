...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        val cell: Cell? = getRandomEmptyCell(board)
        return cell?.let{Pair(cell, generateRandomStartValue())}
    }


    private fun getRandomEmptyCell(board: GameBoard<Int?>): Cell? {
        val freeCells = board.getAllCells().filter { cell -> board[cell] == null }
        return if (freeCells.isEmpty()) null
        else freeCells[(freeCells.indices).random() ]
    }
}